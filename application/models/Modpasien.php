...
class Modpasien extends CI_Model
{
    ...
    {
        $data = $this->db->query("SELECT *, DATE_FORMAT(tanggal_daftar, ('%d %M %Y')) as tanggal_masuk FROM pasien ORDER BY id DESC");
        return $data->result_array();
    }

    function get_data_berobat()
    {
        $data = $this->db->query("SELECT p.*, r.id, r.status, DATE_FORMAT(r.tanggal, ('%d %M %Y')) as tanggal FROM pasien p, rekam_medis r WHERE p.no_medis=r.no_medis AND r.status='antrian' ORDER BY r.id DESC");
        return $data->result_array();
    }
